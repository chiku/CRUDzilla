        ...
        for (key in this.attributes) {
            if (this.attributes.hasOwnProperty(key)) {
                ...
                if (value.mandatory && !Validations.Mandatory(key, value.value)) {
                    this.errors.push({attribute: key, value: value.value, message: Internationalization.missingAttribute.replace('#<>', key.capitalize())});
                }

                if (value.numerical && !Validations.Numerical(key, value.value)) {
                    this.errors.push({attribute: key, value: value.value, message: Internationalization.notNumerical.replace('#<>', key.capitalize())});
                }
            }
        }
